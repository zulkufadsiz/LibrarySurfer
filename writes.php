<?php if ($_GET[action] == "delete") {
        ...
        header("Location: ?page=writes&ok=delete");
        }           
    ?>   
    <?php if (!$_GET[action]) { ?>      
        <div class="card">
            <div class="card-header b-a-0">
                ...
                <h5 class="pull-left"> Sayfa: Makaleler</h5>
                <div class="card-icon pull-right">
                    <a href="?page=writes&amp;action=add" title="Yeni Ekle"><i class="fa fa-plus"></i></a>
                    <a href="?page=writes" title="Tümünü Gör"><i class="fa fa-eye"></i></a>
                </div>  
            </div>
            <div class="card-block">
                <table class="table table-striped table-bordered datatable">
                    ...
                    <tbody>
                    <?php
                        $query_string1        = "SELECT id, status,writer_id,title_tr, image from announcements WHERE writer_id != 0 order by create_date DESC";
                        ...
                        <tr role="row" class="odd">
                            ...
                            <td>
                                <?php 
                                $post_writer_id = $query1->writer_id;
                                $query_string3       = "SELECT id,title_tr from products where id = $post_writer_id";
                            ...
                            <td class="center">
                                <a class="btn btn-info" href="?page=writes&action=edit&id=<?=$query1->id?>">
                                    <i class="fa fa-edit "></i>  
                                </a>
                                <a class="btn btn-danger" href="#" data-toggle="popover" data-trigger="focus hover" data-placement="left" title="Sil?" data-html="true" data-content="<a class='btn btn-danger' href='?page=writes&action=delete&id=<?=$query1->id;?>'>Evet</a>">
    ...
    if ($_GET[action] == "add") {  
        if ($_POST[submit] == "Ekle") {
            ...
            // echo $kategori_sorgu;   
            header("Location: ?page=writes&action=edit&id=$sonid&ok=add");
            echo mysqli_error();
            die();
        }
     ?>
         <div class="card">
            <div class="card-header b-a-0">
                ...
                <h5 class="pull-left"> Sayfa: Makaleler - Ekleme</h5>
                <div class="card-icon pull-right">
                    <a href="?page=writes" title="Tümünü Gör"><i class="fa fa-eye"></i></a>
                </div>  
            </div>
            <div class="card-block">
                <form id="checkForm" data-toggle="validator" role="form" class="form-horizontal" name="form" action="?page=writes&action=add" method="POST" enctype="multipart/form-data" onsubmit="return checkform(this);">
                    <div class="row">
                       ...
                       <div class="form-group col-xs-12 col-sm-3 col-md-3" id="location_selector">
                        ...
                        <div class="form-group col-xs-12 col-sm-3" >
                                    ...
                                    <select class="form-control" name="writer_id" id="exampleSelect1"  required>
                                        <option value="0" selected disabled>Yazar seçiniz.</option>
                                        <?php
                                        // Doktorlar DB' den çekilir.
                                        $query_string2        = "SELECT * FROM products WHERE status = 1 ORDER BY title_tr";
                                        $query_string2_calistir   = mysqli_query($connect, $query_string2);
                                        while ($query2 = mysqli_fetch_object($query_string2_calistir)) {
                                        ?>      
                                            <option value="<?=$query2->id?>"><?=$query2->academic_title_tr . " " .$query2->title_tr?></option>
                                        <?php } ?>
                                    </select>
                                </div>
    ...
    if ($_GET[action] == "edit") {   ?>
        <?php if ($_POST[submit] == "Kaydet") {
            ...
            header("Location: ?page=writes&action=edit&id=$_GET[id]&ok=edit");
            echo mysqli_error();
            }
        ...
        ?>   
         <div class="card">
            <div class="card-header b-a-0">
                ...
                <h5 class="pull-left"> Sayfa: Makaleler - Düzenle</h5>
                <div class="card-icon pull-right">
                    <a href="?page=writes&amp;action=add" title="Yeni Ekle"><i class="fa fa-plus"></i></a>
                    <a href="?page=writes" title="Tümünü Gör"><i class="fa fa-eye"></i></a>
                </div>  
            ...
            <div class="card-block">
                <form id="checkForm" data-toggle="validator" role="form" class="form-horizontal" name="form" action="?page=writes&action=edit&id=<?=$query2->id?>" method="POST" enctype="multipart/form-data" onsubmit="return checkform(this);">
                    <div class="row">
                        <div class="form-group col-xs-12 col-sm-3 col-md-3">
                        ...
                        <div class="form-group col-xs-12 col-sm-3">
                            <label for="exampleSelect1">Yazar<span class="text-danger">*</span></label>
                            <select class="form-control" name="writer_id" id="exampleSelect1" required>
                                <option value="0" disabled selected>Yazar Seçiniz</option>
                                <?php
                                // Doktorlar DB' den çekilir.
                                $query_string3        = "SELECT * FROM products WHERE status = 1 ORDER BY title_tr";
                                $query_string3_calistir   = mysqli_query($connect, $query_string3);
                                while ($query3 = mysqli_fetch_object($query_string3_calistir)) {
                                    ?>
                                    <option value="<?=$query3->id?>" <?php if ($query2->writer_id == $query3->id): ?> selected <?php endif ?>><?=$query3->title_tr?></option>
                                <?php } ?>
                            </select>
                        </div>
