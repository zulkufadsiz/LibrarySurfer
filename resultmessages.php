 <?php if ($_GET[action] == "delete") {
        $sil_sorgu     = "DELETE FROM result_messages WHERE id = $_GET[id]";
        $sil_exec      = mysqli_query($connect, $sil_sorgu);
        header("Location: ?page=resultmessages&ok=delete");
        }           
    ?>  
<?php if (!$_GET[action]) { ?>      
 ...
 <div class="row" >
    <div class="col-lg-4 col-md-6 col-sm-5 ">
            <div class="p-a-0 messages-list bg-white b-r">
              <table class="table messages-datatable">
                    ...
                    <tbody>
                    <?php
                        $query_string1        = "SELECT * FROM result_messages ORDER BY status, create_date DESC";
                        $query_string1_calistir   = mysqli_query($connect, $query_string1);
                        while ($query1 = mysqli_fetch_object($query_string1_calistir)) {
                        ?>  
                        <tr class="result-list-item <?php if ($query1->status == 0){?>bg-info <?php } ?>" id="<?=$query1->id?>" role="row" class="odd">
                            <td class="center">
                            <span class="name"><?=$query1->name; ?> <?=$query1->lname; ?></span>
                            <span class="email"><?=$query1->email?></span>
                                <?=$query1->phone?></td>
                            <td class="center"><?=$query1->create_date ?></td>
                            <td class="center"><a class="btn btn-danger btn-sm" href="#" data-toggle="popover" data-trigger="focus hover" data-placement="left" title="Sil?" data-html="true" data-content="<a class='btn btn-danger btn-sm' href='?page=resultmessages&action=delete&id=<?=$query1->id;?>'>Evet</a>">
    ...
    <div class="col-lg-8 col-md-6 col-sm-7">
        ...
        <?php
            $query_string2        = "SELECT * FROM result_messages ORDER BY status, create_date DESC LIMIT 0,1";
