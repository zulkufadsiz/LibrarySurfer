    <?php if ($_GET[action] == "delete") {
        ...
        $sil_sorgu     = "DELETE FROM news_comments WHERE id = $_GET[id]";
    ...
    <?php if ($_GET[action] == "check") {
            ...
            $query_string1      = "UPDATE news_comments SET 
            status = 1 
            WHERE id = '$_GET[id]'";
            // echo $query_string1;
            $query_string1_calistir = mysqli_query($connect, $query_string1);
            header("Location: ?page=newscomments&ok=edit");
        }           
    ?>            
    <?php if (!$_GET[action]) { ?>      
        <div class="card">
            <div class="card-header b-a-0">
                ...
                <h5 class="pull-left"> Sayfa: Haber Yorumları</h5>
            </div>
            <div class="card-block">
                <table class="table table-striped table-bordered datatable">
                    ...
                    <tbody>
                    <?php
                        $query_string1        = "SELECT * FROM news_comments ORDER BY status,create_date desc";
                        $query_string1_calistir   = mysqli_query($connect, $query_string1);
                        ...
                        <tr role="row" class="odd">
                            ...
                            <td class="center"><a href="http://kocaelifikir.com/haberler/belirsiz/belirsiz/<?=$query1->item_id?>" target="_blank">İçeriğe Git</a></td>
                            <td class="center">
                                <?php
                                    if ($query1->status == 0 ) {  ?> 
                                     <a class="btn btn-success" href="?page=newscomments&action=check&id=<?=$query1->id?>">
