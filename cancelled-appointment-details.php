...
ini_set('display_errors', 0);
    ...
    if ($_POST["message_id"]) {
        ...
        $query2_string       = "SELECT * FROM cancelled_appointment_messages WHERE id = $message_id";
        $query2_string_calistir   = mysqli_query($connect, $query2_string);
        $query2 = mysqli_fetch_object($query2_string_calistir);
       
        if ($query2->status == 0) {
            $query1_string       = "UPDATE cancelled_appointment_messages SET status = '1' WHERE id = $message_id";
