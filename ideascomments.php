    <?php if ($_GET[action] == "delete") {
        ...
        $sil_sorgu     = "DELETE FROM ideas_comments WHERE id = $_GET[id]";
        $sil_exec      = mysqli_query($connect, $sil_sorgu);
        header("Location: ?page=ideascomments&ok=delete");
        }           
    ?>
    <?php if ($_GET[action] == "check") {
            $query_string1      = "UPDATE ideas_comments SET 
            status = 1 
            WHERE id = '$_GET[id]'";
            // echo $query_string1;
            $query_string1_calistir = mysqli_query($connect, $query_string1);
            header("Location: ?page=ideascomments&ok=edit");
        }           
    ?>            
    <?php if (!$_GET[action]) { ?>      
        <div class="card">
            <div class="card-header b-a-0">
                ...
                <h5 class="pull-left"> Sayfa: Fikir Yorumları</h5>
            </div>
            <div class="card-block">
                <table class="table table-striped table-bordered datatable">
                    ...
                    <tbody>
                    <?php
                        $query_string1        = "SELECT * FROM ideas_comments ORDER BY status,create_date desc";
                        $query_string1_calistir   = mysqli_query($connect, $query_string1);
                        ...
                        <tr role="row" class="odd">
                            ...
                            <td class="center">
                                <?php
                                    if ($query1->status == 0 ) {  ?> 
                                     <a class="btn btn-success" href="?page=ideascomments&action=check&id=<?=$query1->id?>">
                                    <i class="fa fa-check "></i>  
                                </a>
                                <?php } ?>
                                <a class="btn btn-danger pull-right" href="#" data-toggle="popover" data-trigger="focus hover" data-placement="left" title="Sil?" data-html="true" data-content="<a class='btn btn-danger' href='?page=ideascomments&action=delete&id=<?=$query1->id;?>'>Evet</a>">
