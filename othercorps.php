    ...
    if ($_GET[action] == "edit") {   ?>
         <div class="card">
            <div class="card-header b-a-0">
                ...
                <h5 class="pull-left"> Sayfa: Anlaşmalı Kuruluşlar</h5>
            </div>
            <div class="card-block">
                ...
                case "": 
                ?>
                    <form method="post" action="?page=other_corps&action=edit&id=1&doc_action=upload" enctype="multipart/form-data">
                    <div class="form-group">
                        <label for="example-color-input" class="col-form-label">Dosyalar</label><br>
                        <label class="control-label" for="focusedInput">Kuruluş görsellerini (CTRL tuşu ile birden fazla yükleyebilirsiniz. Yüklenen dosyanın adı sitede GÖZÜKEN başlık olacaktır.)</label>
                        <div class="controls">
                ...
                <label class="control-label" for="focusedInput">Kuruluşları Sıralayın (Dosyayı sürükleyip bırakarak sıralamayı değiştirebilirsiniz)</label>
                ...
                <div class="row">
                    ...
                    <ul class="sortable-10">
                    <?php

                    $select = "SELECT * FROM other_corps WHERE doc_category = 1 ORDER by doc_ordering"; // select every element in the table in order by order column
                    $perform = mysqli_query($connect,  $select ); //perform selection query
                     ...
                     while( $array = @mysqli_fetch_array( $perform ) ){ //download every row into array
                        ...
                        ?>
                            ...
                            <a class="btn btn-danger" href="?page=other_corps&action=edit&id=1&doc_action=resim_sil&doc_id=<?=$array['doc_id']?>" style="width:100%">Sil</a></li>
                ...
                foreach ($_FILES["imagesUpload"]["error"] as $upload => $error) {//Foreach döngüsü kurarak toplu seçimde array olaran gelen resimleri alýyoruz
                    if ($error == UPLOAD_ERR_OK) {//Resim seçilmiþ ve hata yok ise upload yap
                        ...
                        $query = mysqli_query($connect, "INSERT INTO other_corps (doc_path, doc_title, doc_category) VALUES ('$img_name', '$img_title[0]', '1')");
                        header("Location: ?page=other_corps&action=edit&id=1&ok=edit");
                    }else{//Resim seçilmemiþ ve hata var ise
                        header("Location: ?page=other_corps&action=edit&id=1&ok=error");
                    }
                }
                break;
                ...
                case "resim_sil":
                $id = $_GET['doc_id'];
                $silincek_resim = mysqli_query($connect, "SELECT * FROM other_corps WHERE doc_id=$id");
                $silincek_resim_yeni = mysqli_fetch_array($silincek_resim);
                $sil = mysqli_query($connect, "DELETE FROM other_corps WHERE doc_id=$id");

                if($sil){
                header("Location: ?page=other_corps&action=edit&id=1&ok=delete");
