<!--     <?php if ($_GET[action] == "delete") {
       // remove_featured_pool(1418, 4);

        $sil_sorgu     = "delete from news where id = 1418";
        $sil_exec      = mysqli_query($connect, $sil_sorgu);
        //header("Location: ?page=calisma&ok=delete");
        }           
    ?>  -->         
    <?php if (!$_GET[action]) { ?>      
        <div class="card">
            <div class="card-header b-a-0">
                ...
                <div class="card-icon pull-right">
                    <a href="?page=calisma&amp;action=add" title="Yeni Ekle"><i class="fa fa-plus"></i></a>
                    <a href="?page=calisma" title="Tümünü Gör"><i class="fa fa-eye"></i></a>
                </div>  
            </div>
                ...
                <table class="table table-striped table-bordered datatable">
                    ...
                    <?php
                        ...
                        <tr role="row" class="odd">
                            ...
                            <td class="center">
                                <a class="btn btn-info" href="?page=calisma&action=edit&id=<?=$query1->id?>">
                                    <i class="fa fa-edit "></i>  
                                </a>
                                <!-- <a class="btn btn-danger" href="#" data-toggle="popover" data-trigger="focus hover" data-placement="left" title="Sil?" data-html="true" data-content="<a class='btn btn-danger' href='?page=calisma&action=delete&id=<?=$query1->id;?>'>Evet</a>">
    ...
    if ($_GET[action] == "add") {  
        if ($_POST[submit] == "Ekle") {
         ...
         if ($_FILES[catalog_tr][name]) {
            ...
            header("Location: ?page=calisma&action=edit&id=$sonid&ok=add");
            echo mysqli_error();
            die();
        }
     ?>
         <div class="card">
            ...
            <div class="card-block">
                <form id="checkForm" data-toggle="validator" role="form" class="form-horizontal" name="form" action="?page=calisma&action=add" method="POST" enctype="multipart/form-data" onsubmit="return checkform(this);">
                    <div class="row">
    ...
    if ($_GET[action] == "edit") {   ?>
        <?php if ($_POST[submit] == "Kaydet") {
           ...
           $sorgu      = "update news set 
            ...
            author = '$author',
            reporter = '$reporter'
            $resim $resim2 $dosya where id = '1418'";
            // echo $sorgu;
            $sorgu_calistir = mysqli_query($connect, $sorgu);    

            if ($location_type != 1) {
            $category_type = 1; // Havuza eklenecek kategori tipi "Haber"
            add_featured_pool(1418,$location_type,$category_type);
            }
            else if($status == 0){
                remove_featured_pool(1418, 1);
            }

            // header("Location: ?page=calisma&action=edit&id=1418&ok=edit");
            echo mysqli_error();
            }
        ...
        <?php
                // Düzenle
        $query_string2        = "SELECT * FROM news WHERE id = '1418'";
        ...
        ?>   
         <div class="card">
            ...
            <div class="card-block">
                <form id="checkForm" data-toggle="validator" role="form" class="form-horizontal" name="form" action="?page=calisma&action=edit&id=<?=$query2->id?>" method="POST" enctype="multipart/form-data" onsubmit="return checkform(this);">
                    <div class="row">
                        <div class="form-group col-xs-12 col-sm-3 col-md-3">
                ...
                <hr>
                              <!-- Çoklu Görsel Yükleme Betiği -->
                                ...
                                case "": 
                                ?>
                                    <form method="post" action="?page=calisma&action=edit&id=<?=$_GET['id']?>&pp_action=upload" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    ...
                                    $select = "SELECT * FROM news_pictures WHERE product_id = 1418 ORDER by picture_ordering"; // select every element in the table in order by order column
                                    $perform = mysqli_query($connect,  $select ); //perform selection query
                                     ...
                                     while( $array = @mysqli_fetch_array( $perform ) ){ //download every row into array
                                        ...
                                        ?>
                                            ...
                                            <a class="btn btn-danger" href="?page=calisma&action=edit&id=<?=1418?>&pp_action=resim_sil&picture_id=<?=$array['picture_id']?>" style="width:100%">Sil</a></li>
                                ...
                                foreach ($_FILES["imagesUpload"]["error"] as $upload => $error) {//Foreach döngüsü kurarak toplu seçimde array olaran gelen resimleri alýyoruz
                                    if ($error == UPLOAD_ERR_OK) {//Resim seçilmiþ ve hata yok ise upload yap
                                        ...
                                        $query = mysqli_query($connect, "INSERT INTO news_pictures (picture_path, product_id) VALUES ('$img_name', '1418')");
                                        header("Location: ?page=calisma&action=edit&id=1418&ok=edit");
                                    }else{//Resim seçilmemiþ ve hata var ise
                                        header("Location: ?page=calisma&action=edit&id=1418&ok=error");
                                    }
                                }
                                break;
                                ...
                                if($sil){
                                header("Location: ?page=calisma&action=edit&id=1418&ok=delete");
